...
interface GradualBlurProps {
  ...
  // Added responsive props for the hook
  mobileHeight?: string;
  tabletHeight?: string;
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      if (config.exponential) {
        blurValue = Math.pow(2, progress * 4) * 0.0625 * currentStrength * 10;
      } else {
        blurValue = 0.0625 * (progress * config.divCount + 1) * currentStrength * 10;
      }
