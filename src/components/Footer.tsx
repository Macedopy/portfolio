'use client';
import { useLanguage } from '@/context/LanguageContext';

export default function Footer() {
  const { t } = useLanguage();

  return (
    <footer style={{ padding: '100px 0 32px', borderTop: '1px solid var(--border-color)' }}>
      <div className="container">
        <div style={{ 
          display: 'grid', 
          gridTemplateColumns: 'repeat(2, 1fr)', 
          gap: '64px',
          marginBottom: '64px'
        }}>
          {/* Col 1 */}
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '24px' }}>
              <div style={{ width: '24px', height: '24px', background: 'var(--accent-primary)', borderRadius: '4px' }}></div>
              <span style={{ fontWeight: 700, fontSize: '18px', letterSpacing: '0.05em' }}>{t('footer.col1.title')}</span>
            </div>
            <p style={{ fontSize: '14px', color: 'var(--secondary)', lineHeight: '1.6', marginBottom: '24px' }}>
              {t('footer.col1.desc')}
            </p>
          </div>

          {/* Col 4 (Now 2nd) */}
          <div id="client">
            <h4 style={{ fontSize: '20px', fontWeight: 700, marginBottom: '24px' }}>{t('footer.col4.title')}</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', fontSize: '18px', color: 'var(--secondary)' }}>
              <span>📞 +55 19 98350-6250</span>
              <span>✉️ brunomacedolemos15@gmail.com</span>
            </div>
          </div>
        </div>

        <div style={{ 
          borderTop: '1px solid var(--border-color)', 
          paddingTop: '32px', 
          textAlign: 'center',
          fontSize: '14px',
          color: 'var(--secondary)'
        }}>
          Copyright 2022 | {t('footer.rights')}
        </div>
      </div>
    </footer>
  );
}
